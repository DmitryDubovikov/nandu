...
def is_prose(t):
    ...
    if t.endswith(('.', '!', '?', ':', ';', ',')):
        return True
    # Ёлочки — признак прозы только там, где нет знака отношения: строка
    # P(блока «A, B, C, D, A, B») = 288 — это запись, а не фраза, и терять
    # из-за кавычек в имени фигуры итог задачи нельзя.
...
CARDS = [
    ...
    (r'^\*\*Пример ',                    'exam',   None),
    (r'^\*\*Задача[.:]',                  'task',   'Задача'),
    (r'^\*\*Условие[.:]',                 'task',   'Условие'),
    (r'^\*\*Шаг \d',                     'step',   None),
]
